...
public:
    ...
    vector<BedTarget*> targetsContained(BedTarget& target) {
        const vector<Interval<size_t, BedTarget*> > results = intervals[target.seq].findContained(target.left, target.right);
    ...
    vector<BedTarget*> targetsOverlapping(BedTarget& target) {
        const vector<Interval<size_t, BedTarget*> > results = intervals[target.seq].findOverlapping(target.left, target.right);
